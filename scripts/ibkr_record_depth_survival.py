#!/usr/bin/env python3
"""Phase 12a: read-only OSE L2 recorder for touch-survival research.

Requires a successful Phase 11 capability summary. Resolves one candidate's unique
option legs, subscribes to live L1 plus direct market depth, reconstructs the visible
book from insert/update/delete callbacks, and writes event-level top-of-book states.
...
class DepthBook:
    ...
    def apply(self, *, position: int, operation: int, side: int, price: Any, size: Any,
              market_maker: str = "") -> bool:
        """Apply an IBKR depth row operation. operation: 0 insert, 1 update, 2 delete.

        side: 0 ask, 1 bid. Returns False for invalid position/operation instead of
        corrupting the local book.
        """
...
def require_phase11_ready(path: str | Path) -> dict[str, Any]:
    data = json.loads(Path(path).read_text(encoding="utf-8"))
    decision = _text(data.get("phase11_overall_decision"))
    if decision != "READY_FOR_L2_FILL_MODEL":
        raise ValueError(
            f"Phase 11 is not ready for L2 modelling: {decision or 'missing decision'}"
        )
    return data
...
def make_recorder_app():
    ...
    class App(EWrapper, EClient):
        ...
        def _record_depth(self, req_id: int, position: int, operation: int, side: int,
                          price: Any, size: Any, market_maker: str, is_l2: bool) -> None:
            now_mono = time.monotonic()
            with self.data_lock:
                book = self.books.setdefault(req_id, DepthBook())
                applied = book.apply(
                    position=int(position), operation=int(operation), side=int(side),
                    price=price, size=size, market_maker=market_maker,
                )
                if not applied or req_id not in self.recording_enabled:
                    return
                meta = self.depth_meta.get(req_id, {})
                start = self.recording_start_mono or now_mono
                bid, bid_size, ask, ask_size = book.top()
                l1_req = meta.get("l1_req_id")
                md_type = self.market_data_type.get(int(l1_req)) if l1_req is not None else None
                self.events.append({
                    **meta,
                    ...
                    "callback_kind": "L2" if is_l2 else "L1_DEPTH",
        ...
        def updateMktDepth(self, reqId, position, operation, side, price, size):  # noqa: N802, ANN001
            self._record_depth(int(reqId), position, operation, side, price, size, "", False)

        def updateMktDepthL2(self, reqId, position, marketMaker, operation, side, price, size, isSmartDepth):  # noqa: N802, ANN001, ARG002
            self._record_depth(int(reqId), position, operation, side, price, size, str(marketMaker), True)
...
def record_candidate(candidate: dict[str, Any], *, app, Contract, exchange: str,
                     currency: str, timeout: float, warmup_sec: float,
                     duration_sec: float, depth_rows: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:  # noqa: N803, ANN001
    ...
    # Sequential depth subscriptions intentionally stay below the smallest documented
    # default depth-line cap and avoid accidental over-subscription on 4-leg candidates.
    for leg in unique.values():
        contract = resolve_option_contract(
            app, Contract, underlying=underlying, expiry=expiry, leg=leg,
            exchange=exchange, currency=currency, timeout=timeout,
        )
        ...
        with app.data_lock:
            ...
            book = app.books.get(depth_id, DepthBook())
            bid, bid_size, ask, ask_size = book.top()
            if app.recording_start_mono is None:
                app.recording_start_mono = time.monotonic()
            app.recording_enabled.add(depth_id)
            # Baseline state ensures a quiet but stable book is represented.
            app.events.append({
                **meta,
                "elapsed_ms": round((time.monotonic() - app.recording_start_mono) * 1000.0, 3),
                "observed_at_utc": datetime.now(timezone.utc).isoformat(),
                "position": -1,
                "operation": -1,
                "side": -1,
        ...
        with app.data_lock:
            ...
            app.events.append({
                **meta,
                "elapsed_ms": round((time.monotonic() - (app.recording_start_mono or time.monotonic())) * 1000.0, 3),
                "observed_at_utc": datetime.now(timezone.utc).isoformat(),
                "position": -1,
                "operation": -1,
                "side": -1,
            ...
            event_count = sum(1 for e in app.events if e.get("depth_req_id") == depth_id)
        try:
    ...
    all_have_events = bool(per_leg) and all(int(r["recorded_events"]) > 0 for r in per_leg)
    decision = (
        "DEPTH_TRACE_READY_FOR_SURVIVAL_ANALYSIS"
        if all_live and all_have_events
        else "DEPTH_TRACE_INCOMPLETE"
    )
    summary = {
        "phase12_recording_decision": decision,
        ...
        "per_leg": per_leg,
        "interpretation": (
            "This trace measures displayed depth state changes only. It does not reveal queue position, "
            "hidden liquidity, order priority, or actual fill probability."
        ),
    }
    return rows, summary
...
def _args() -> argparse.Namespace:
    ...
    p.add_argument("--phase11-summary", required=True)
    ...
    p.add_argument("--duration-per-leg", type=float, default=30.0)
    p.add_argument("--depth-rows", type=int, default=5)
    p.add_argument("--output", default="data/phase12_depth_events.csv")
    p.add_argument("--summary-json", default="data/phase12_recording_summary.json")
    args = p.parse_args()
...
def main() -> int:
    args = _args()
    require_phase11_ready(args.phase11_summary)
    ...
    try:
        rows, summary = record_candidate(
            candidate, app=app, Contract=Contract, exchange=args.exchange,
            currency=args.currency, timeout=args.timeout, warmup_sec=args.warmup,
            duration_sec=args.duration_per_leg, depth_rows=args.depth_rows,
        )
    finally:
        app.disconnect()
